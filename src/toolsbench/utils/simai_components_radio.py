...
from src.toolsbench.utils.deepinv_imager import MyRadioInterferometry
...
def _run_pnp_updates(
    ...
    with torch.no_grad():
        for _ in range(int(pnp_cfg["inner_iterations"])):
            ...
            sig_min, sig_max = pnp_cfg["min_pixel"], pnp_cfg["max_pixel"]
            scale = sig_max - sig_min
            reconstruction = (reconstruction - sig_min) / scale
            ...
            if pnp_cfg["denoiser_lambda_relaxation"] is None:
                ...
                reconstruction = reconstruction * scale + sig_min
...
def _fits_to_tensor(fits_path):
    with fits.open(fits_path, memmap=False) as hdul:
        img_np = hdul[0].data.astype(np.float32)
        img = torch.from_numpy(img_np)
    return _normalize_image_tensor(img)


def _read_measurement_from_path(measurement_path):

    with table(measurement_path, readonly=True) as tb:
        visibilities = tb.getcol("DATA")

    return torch.from_numpy(visibilities)
    ...
    try:
        for packet_id, sample in enumerate(source_iter):
            ...
            if sample_physics_spec != current_physics_spec or current_physics is None:
                current_physics = _build_physics_from_spec_radio(
            ...
            with torch.no_grad():
                ...
                if measurement_path is not None:
                    y = _read_measurement_from_path(measurement_path)
                else:
...
def _build_physics_from_spec_radio(ground_truth_shape, physics_spec, compute_device):
    imaging_npixel = int(ground_truth_shape[-1])
    samples_locs = physics_spec["samples_locs"]
...
def pnp_consumer_component(
    ...
    physics = _build_physics_from_spec_radio(
    ...
    while True:
        ...
        if hasattr(physics, "A_adjoint"):
            with torch.no_grad():
                weights = batch_physics_spec.get("weights", None)
                if weights is not None:
                    # Create a temporary weighted operator for a sharper init
                    weighted_op = copy.deepcopy(physics)
                    weighted_op.setWeight(weights.to(compute_device))
                    reconstruction_batch = weighted_op.A_adjoint(measurement)
                else:
                    reconstruction_batch = physics.A_adjoint(measurement)

                    sig_min, sig_max = pnp_cfg["min_pixel"], pnp_cfg["max_pixel"]
                    # Peak-normalize dirty image to signal range and clamp
                    reconstruction_batch = reconstruction_batch * sig_max / reconstruction_batch.max()
                    reconstruction_batch = reconstruction_batch.clamp(sig_min, sig_max)
